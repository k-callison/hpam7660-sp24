...
9. Now do the same conversion and preview for the `airline_safety` data frame from the `fivethirtyeight` package. (Hint: you want the tidy data frame to include the variable `fatalities_years` that indicates the time period and the variable `count` that measures the fatality counts).
...
11. Preview the data using one of the methods we covered in [Tutorial 2](/assignments/tutorial_2.qmd).

12. Is this data frame in "tidy" format? If yes, then explain why. If not, then convert the data to "tiny" format.

13. Once you've finished Step 12, knit your PDF document and push it to your GitHub repo. Make sure the document shows up in the repo, invite me to the repo, and you're done!