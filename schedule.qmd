...
The detailed course schedule for the semester is below. Please note that dates and topics are subject to change and will depend on the pace of the course. Please stay apprised of any changes announced in class. You can find the materials for each course meeting under the “Content” links for that week. You should generally: 
   
- watch the lecture videos (if any) and complete the readings by Tuesday;
- complete the tutorials by Monday evening at 11:59pm; and
- submit the problem set or exam by Thursday at 11:59pm.
                
Here's a guide to the schedule: 

- [**Materials**](/materials/) ({{< fa book-open-reader >}}): This page contains the readings, slides, and videos (if any) for the topic. Do the readings and watch the videos before coming to class on the indicated date. Slides will typically be posted after each class.
...
schedule <- schedule %>%  
    ...
    mutate(col_date = ifelse(is.na(date_end),
                             glue('<span>{format(date, "%B %e")}</strong>'),
                             glue('<span>{format(date, "%B %e")}</strong>–<strong>{format(date_end, "%B %e")}</strong>'))) %>%
