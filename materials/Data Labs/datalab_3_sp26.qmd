---
title: "Data Lab 3 - Importing and Describing Data"
summary: >-
  Exploring Family Connects New Orleans Data.
---
...
Start by downloading a csv file of the FCNO data using this link [FCNO Data](https://www.dropbox.com/scl/fi/ghrkqfg0nh930omnvd57o/fcno_data.csv?rlkey=zzrr8wm280hp9mf8d8mtbrptm&st=c7ga96el&dl=1).
...
```{r, eval=FALSE}
fcno_data = read.csv("PATH/fcno_data.csv", header = TRUE)
```
...
```{r, eval=FALSE}
fcno_data <- read.csv("C:\\Users\\kevin\\data\\fcno_data.csv", header=TRUE)
```

## Step 5: Evaluating and Cleaning the Data
...
Note that the first column is labeled "patient_id". That data in that column is just what it sounds like - a number that identifies an individual patient. Because most people receive multiple medical services over the course of their time in Medicaid, most patient_ids will be repeated several times in the data.
...
```{r, eval=FALSE}
range(fcno_data$days_from_delivery)
```

You should see that the values of "days_from_delivery" range from -280 to 365. This means that we're observing medical services received up to 9 months (280 days) prior to delivery and up to 1 year post-delivery.
...
```{r, eval=FALSE}
range(fcno_data$revenue_code, na.rm = TRUE)
```

When you run this command, you should see that the revenue codes range from 1 to 999.
...
You can see that most of the women giving birth in the data are between the ages of 22 and 31, with counts trailing off as we get to younger and older ages.
...
Note that we need to first load the `dplyr` library since the `distinct()` and `count()` functions and the pipe operator are all part of the `dplyr` package.
...
summary(prenatal_spend$sum_spend[prenatal_spend$fcno == 0])
```
