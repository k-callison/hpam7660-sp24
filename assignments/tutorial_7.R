...
cancer_plot_white <- cancer_alley_rates %>%
  filter(black == 0)

cancer_plot_black <- cancer_alley_rates %>%
  filter(black == 1)

ggplot(data = cancer_plot_black, mapping = aes(x = year, y = lung_rate_adj_wt, linetype = as.factor(cancer_parish))) +
  geom_line()
